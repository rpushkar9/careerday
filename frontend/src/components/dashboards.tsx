...
export default function Year1Dashboard({ 
  ...
  // ✅ Helper: safely compute total credits/costs if not provided
  const safeTotalCredits =
    data?.totalYearCredits ??
    (data?.fall?.totalCredits ?? 0) + (data?.spring?.totalCredits ?? 0);

  const safeTotalCost =
    data?.totalYearCost ??
    (data?.fall?.tuition ?? 0) + (data?.spring?.tuition ?? 0);
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-12 relative z-10">
        <div className="grid md:grid-cols-4 gap-6 mb-12">
          <StatCard
            icon={BookOpen}
            title="Total Credits"
            value={safeTotalCredits.toString()}
            subtitle="For Year 1"
            color="#6d6bd3"
          />
          <StatCard
            icon={DollarSign}
            title="Total Cost"
            value={`$${safeTotalCost.toLocaleString()}`}
