...
import { GraduationCap, Sparkles, ArrowRight, Download, RefreshCw, Heart, Save, Check } from 'lucide-react';
...
export default function RoadmapPage() {
  ...
  useEffect(() => {
    ...
    if (userStr) {
      ...
      const existingRoadmap = savedRoadmaps.find(r => r.careerTitle === careerTitle);
      if (existingRoadmap) {
        setIsSaved(true);
  ...
  const generateAiRoadmap = async () => {
    ...
    try {
      ...
      alert('Failed to generate AI roadmap. Make sure the backend is running on port 5002.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLike = () => {
    ...
    if (userStr) {
      const user = JSON.parse(userStr);
      const savedRoadmaps = user.savedRoadmaps || [];
      const roadmapIndex = savedRoadmaps.findIndex(r => r.careerTitle === careerTitle);
  ...
  const handleSave = () => {
    ...
    const existingIndex = savedRoadmaps.findIndex(r => r.careerTitle === careerTitle);
  ...
  const handleViewYear1 = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5002/api/scrape-year1-data', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {showSaveSuccess && (
          ...
          >
            ...
            <span className="font-semibold">Roadmap saved to your dashboard!</span>
          </motion.div>
        )}
